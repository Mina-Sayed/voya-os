...
async function loadApprovalRequests(membership: Awaited<ReturnType<typeof requireWorkspaceMembership>>): Promise<ApprovalRequestItem[]> {
  const client = await createServerSupabaseClient(); const { data, error } = await client.rpc("list_approval_requests", { p_organization_id: membership.organizationId, p_limit: 50 }); if (error) throwWorkspaceOperationError("workspace.read", error); return ((data ?? []) as { id: string; resource_type: string; resource_id: string; proposed_action: string; status: ApprovalRequestItem["status"]; expires_at: string | null; created_at: string }[]).map((item) => ({ id: item.id, resourceId: item.resource_id, resourceType: item.resource_type, proposedAction: item.proposed_action, status: item.status, expiresAt: item.expires_at, createdAt: item.created_at }));
}
