...
async function loadBookingOptions(membership: Awaited<ReturnType<typeof requireWorkspaceMembership>>): Promise<{ properties: BookingDraftOption[]; clients: BookingDraftOption[]; drafts: BookingDraftListItem[]; currency: string }> {
  ...
  const now = Date.now();
  const latestExecutableAmendmentByBooking = new Map<string, string>();
  for (const item of ((approvalsResult.data ?? []) as { id: string; resource_id: string; proposed_action: string; status: string; expires_at: string | null }[])) {
    ...
    if (
      ...
      && expiresAt > now
      && !latestExecutableAmendmentByBooking.has(item.resource_id)
    ) latestExecutableAmendmentByBooking.set(item.resource_id, item.id);
  }
  return {
    ...
    drafts: ((draftsResult.data ?? []) as { id: string; property_code: string; property_name: string; client_name: string | null; status: BookingDraftListItem["status"]; check_in: string; check_out: string; agreed_total_amount_minor: string | null; currency: string | null; commercial_completion_status: BookingDraftListItem["commercialCompletionStatus"]; version: number; has_check_in: boolean; has_check_out: boolean; created_at: string }[]).map((item) => {
      const executableAmendmentId = latestExecutableAmendmentByBooking.get(item.id) ?? null;
      return { id: item.id, propertyLabel: `${item.property_code} — ${item.property_name}`, clientLabel: item.client_name ?? "عميل غير مرتبط", status: item.status, checkIn: item.check_in, checkOut: item.check_out, amountMinor: item.agreed_total_amount_minor, currency: item.currency, commercialCompletionStatus: item.commercial_completion_status, version: item.version, hasCheckIn: item.has_check_in, hasCheckOut: item.has_check_out, createdAt: item.created_at, latestApprovedAmendmentId: executableAmendmentId, latestApprovedAmendmentIsExecutable: executableAmendmentId !== null };
    }),
