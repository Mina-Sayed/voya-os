...
import { selectLatestExecutableAmendments } from "@/features/bookings/executable-amendments";
...
async function loadBookingOptions(membership: Awaited<ReturnType<typeof requireWorkspaceMembership>>): Promise<{ properties: BookingDraftOption[]; clients: BookingDraftOption[]; drafts: BookingDraftListItem[]; currency: string }> {
  ...
  const latestExecutableAmendmentByBooking = selectLatestExecutableAmendments(
    (approvalsResult.data ?? []) as { id: string; resource_id: string; proposed_action: string; status: string; expires_at: string | null }[],
  );
  return {
