...
const deterministicBookingErrors = new Set(["22003", "22008", "22023", "22P02", "23503", "23514", "23P01"]);
...
export async function createBookingDraftAction(_previousState: BookingDraftState, formData: FormData): Promise<BookingDraftState> {
  ...
  const checkIn = formValue(formData, "check_in");
  const checkOut = formValue(formData, "check_out");
  const amountMajor = formValue(formData, "amount_major");
  ...
  if (!propertyId || !clientId || !isValidIsoDate(checkIn) || !isValidIsoDate(checkOut) || !amountMinor || !currency || !idempotencyKey || checkIn >= checkOut) {
    return { status: "invalid", message: "أكمل العقار والعميل والتواريخ والمبلغ والعملة بشكل صحيح." };
  }
  ...
  try {
    ...
    const { error } = await client.rpc("create_commercial_booking_draft", {
    ...
    if (error) {
      if (error.code === "42501") return { status: "denied", message: "لا تملك صلاحية إنشاء مسودة حجز." };
      if (error.code === "23505") return invalidWithFreshKey("تحقق من بيانات المسودة ثم أعد المحاولة.");
      if (deterministicBookingErrors.has(error.code ?? "")) return { status: "invalid", message: "تحقق من بيانات المسودة ثم أعد المحاولة." };
      reportWorkspaceActionFailure("workspace.booking.create", error, requestId);
      return { status: "retry", message: "تعذر حفظ المسودة الآن. حاول مرة أخرى." };
    }
    revalidatePath("/workspace/bookings");
    return { status: "success", message: "تم إنشاء مسودة الحجز التجاري." };
  } catch (error) {
    reportWorkspaceActionFailure("workspace.booking.create", error, requestId);
...
function lifecycleValue(formData: FormData, key: string) {
