  ...
  return (
    <form action={formAction} className="mt-4 grid gap-3 border-t border-line pt-4" ref={formRef}>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <label className="text-xs font-bold text-harbor">المنطقة الزمنية<select className="ltr mt-1 h-10 w-full rounded-lg border border-[#c9d9d3] bg-white px-3 text-xs" defaultValue={property.timezone} disabled={isPending} name="timezone" required>{SUPPORTED_TIMEZONES.map((timezone) => <option key={timezone} value={timezone}>{timezone}</option>)}</select></label>
        ...
        <label className="text-xs font-bold text-harbor">السعر اليومي<input className="mt-1 h-10 w-full rounded-lg border border-[#c9d9d3] bg-white px-3 font-mono text-xs" defaultValue={property.dailyPrice ?? ""} disabled={isPending} min="0" name="daily_price" step="0.001" type="number" /></label>
        <label className="text-xs font-bold text-harbor">السعر الأسبوعي<input className="mt-1 h-10 w-full rounded-lg border border-[#c9d9d3] bg-white px-3 font-mono text-xs" defaultValue={property.weeklyPrice ?? ""} disabled={isPending} min="0" name="weekly_price" step="0.001" type="number" /></label>
        <label className="text-xs font-bold text-harbor">السعر الشهري<input className="mt-1 h-10 w-full rounded-lg border border-[#c9d9d3] bg-white px-3 font-mono text-xs" defaultValue={property.monthlyPrice ?? ""} disabled={isPending} min="0" name="monthly_price" step="0.001" type="number" /></label>
        <label className="text-xs font-bold text-harbor">العملة<select className="mt-1 h-10 w-full rounded-lg border border-[#c9d9d3] bg-white px-3 text-xs" defaultValue={property.currency ?? ""} disabled={isPending} name="currency"><option value="">بدون عملة</option>{SUPPORTED_CURRENCIES.map((currency) => <option key={currency.code} value={currency.code}>{currency.label}</option>)}</select></label>
