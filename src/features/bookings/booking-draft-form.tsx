...
export function BookingDraftForm({ createDraft, properties, clients, currency }: Readonly<{ createDraft: BookingDraftAction; properties: readonly BookingDraftOption[]; clients: readonly BookingDraftOption[]; currency: string }>) {
  ...
  return (
    <form action={formAction} className="rounded-[1.75rem] border border-[#d4dfda] bg-[#f0f7f4] p-5 shadow-[0_18px_44px_rgba(16,33,38,0.04)] sm:p-7" ref={formRef}>
      ...
      <div className="flex items-start gap-3"><div className="grid size-10 place-items-center rounded-xl bg-harbor text-sea-glass"><CalendarPlus aria-hidden="true" className="size-5" /></div><div><p className="text-[11px] font-bold tracking-[0.08em] text-tide">طلب تشغيل تجاري</p><h1 className="mt-1 text-2xl font-bold tracking-[-0.08em] text-harbor">مسودة حجز</h1><p className="mt-2 text-sm leading-6 text-muted">يُدخل المستخدم المبلغ بوحدة العملة الرئيسية، ويحوّله الخادم بدقة إلى أصغر وحدة قبل التخزين.</p></div></div>
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <label className="text-xs font-bold text-harbor" htmlFor="booking-amount">المبلغ المتفق عليه ({currency})<input className="ltr mt-2 h-12 w-full rounded-xl border border-[#c9d9d3] bg-white px-3 text-sm font-normal text-ink outline-none focus:border-tide focus:ring-4 focus:ring-sea-glass/35 disabled:bg-canvas" disabled={isPending || !isReady} id="booking-amount" inputMode="decimal" min="0" name="amount" pattern="[0-9]+([.][0-9]{1,3})?" placeholder="25000.00" required type="text" /></label>
        <label className="text-xs font-bold text-harbor" htmlFor="booking-currency">العملة<input name="currency" type="hidden" value={currency} /><select className="mt-2 h-12 w-full rounded-xl border border-[#c9d9d3] bg-white px-3 text-sm font-normal text-ink outline-none disabled:bg-canvas" defaultValue={currency} disabled id="booking-currency"><option value={currency}>{currency}</option></select></label>
      </div>
