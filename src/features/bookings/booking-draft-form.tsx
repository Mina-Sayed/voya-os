...
export function BookingDraftForm({ createDraft, properties, clients, currency }: Readonly<{ createDraft: BookingDraftAction; properties: readonly BookingDraftOption[]; clients: readonly BookingDraftOption[]; currency: string }>) {
  ...
  const currencySupported = decimalDigits !== null;
  const amountPattern = decimalDigits === null ? undefined : decimalDigits === 0 ? "(?:0|[1-9]\\d*)" : `(?:0|[1-9]\\d*)(?:\\.\\d{1,${decimalDigits}})?`;
  const amountStep = decimalDigits === null ? undefined : decimalDigits === 0 ? "1" : `0.${"0".repeat(decimalDigits - 1)}1`;
  return (
      ...
      <div className="flex items-start gap-3"><div className="grid size-10 place-items-center rounded-xl bg-harbor text-sea-glass"><CalendarPlus aria-hidden="true" className="size-5" /></div><div><p className="text-[11px] font-bold tracking-[0.08em] text-tide">طلب تشغيل تجاري</p><h1 className="mt-1 text-2xl font-bold tracking-[-0.08em] text-harbor">مسودة حجز</h1><p className="mt-2 text-sm leading-6 text-muted">يُحفظ المبلغ المتفق عليه كـ snapshot تجاري. لا يعني ذلك أنه مدفوع أو أنه إيراد.</p></div></div>
      ...
      {!currencySupported ? <p className="mt-4 text-xs leading-6 text-coral">عملة المؤسسة غير موجودة في عقد العملات المدعومة؛ صحّح الإعداد قبل إنشاء حجز.</p> : null}
