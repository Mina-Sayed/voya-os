...
function BookingCommand({ bookingId, label, action, kind }: Readonly<{ bookingId: string; label: string; action: BookingLifecycleAction; kind: "approval" | "confirm" }>) {
  ...
  return <form action={formAction} className="inline-flex flex-col items-start" ref={formRef}><input name="booking_id" type="hidden" value={bookingId} /><input name="idempotency_key" type="hidden" value={idempotencyKey} /><button className={`inline-flex min-h-10 items-center gap-1.5 rounded-xl px-3 text-[11px] font-bold disabled:opacity-50 ${kind === "approval" ? "border border-[#d8c9a4] bg-white text-[#85652e] hover:bg-[#fff8e9]" : "bg-tide text-white hover:bg-harbor"}`} disabled={pending} type="submit">{kind === "approval" ? <ShieldCheck aria-hidden="true" className="size-3.5" /> : <CheckCircle2 aria-hidden="true" className="size-3.5" />}{label}</button><ActionFeedback state={state} /></form>;
}
...
function BookingCard({ booking, properties, clients, currency, requestApproval, confirmBooking, requestAmendment, executeAmendment, recordStay, canOperateStay, canApprove }: Readonly<{ booking: BookingDraftListItem; properties: readonly BookingDraftOption[]; clients: readonly BookingDraftOption[]; currency: string; requestApproval: BookingLifecycleAction; confirmBooking: BookingLifecycleAction; requestAmendment?: BookingLifecycleAction; executeAmendment?: BookingLifecycleAction; recordStay: BookingStayAction; canOperateStay: boolean; canApprove: boolean }>) {
  ...
  return <article className="rounded-2xl border border-[#e5e9e4] bg-[#fcfdfb] p-4">
    ...
    <div className="mt-4 flex flex-wrap gap-2">
      ...
      {booking.status === "confirmed" && canApprove && executeAmendment ? <BookingCommand action={executeAmendment} bookingId={booking.id} kind="confirm" label="تطبيق تعديل معتمد" /> : null}
    </div>
    {booking.status === "confirmed" && requestAmendment ? <AmendmentForm action={requestAmendment} booking={booking} clients={clients} currency={currency} properties={properties} /> : null}
  </article>;
}

export function BookingsPage({ properties, clients, drafts, createDraft, requestApproval, confirmBooking, requestAmendment, executeAmendment, recordStay, canOperateStay, canApprove, currency }: Readonly<{ properties: readonly BookingDraftOption[]; clients: readonly BookingDraftOption[]; drafts: readonly BookingDraftListItem[]; createDraft: BookingDraftAction; requestApproval: BookingLifecycleAction; confirmBooking: BookingLifecycleAction; requestAmendment?: BookingLifecycleAction; executeAmendment?: BookingLifecycleAction; recordStay: BookingStayAction; canOperateStay: boolean; canApprove: boolean; currency: string }>) {
