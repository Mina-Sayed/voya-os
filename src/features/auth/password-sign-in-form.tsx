...
import type { PasswordSignInStatus } from "./password-sign-in";

type PasswordSignInFormProps = Readonly<{
  configured: boolean;
  onSignIn(email: string, password: string): Promise<Readonly<{ status: PasswordSignInStatus | "unavailable" }>>;
...
export function PasswordSignInForm({
  ...
  const [status, setStatus] = useState<PasswordSignInStatus | "unavailable" | null>(configured ? null : "unavailable");
  ...
  return (
    <form className="mt-8 space-y-4" noValidate onSubmit={handleSubmit}>
      ...
      <p className="text-xs leading-6 text-muted">لا تعرف كلمة المرور؟ <a className="font-bold text-tide underline decoration-dotted underline-offset-4" href="#magic-link">استخدم رابط الدخول الآمن</a>.</p>
    </form>
  );
}
