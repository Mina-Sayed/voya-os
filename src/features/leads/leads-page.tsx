...
function LeadCard({ lead, updateLead, archiveLead, createActivity, createFollowUp, completeFollowUp, convertLead }: Readonly<{ lead: LeadItem; updateLead?: CrmCommandAction; archiveLead?: CrmCommandAction; createActivity?: CrmCommandAction; createFollowUp?: CrmCommandAction; completeFollowUp?: CrmCommandAction; convertLead?: CrmCommandAction }>) {
  ...
  return <article className={`relative overflow-hidden rounded-[1.5rem] border bg-surface p-5 shadow-[0_10px_28px_rgba(16,33,38,0.035)] ${archived ? "border-[#e3d8d2] opacity-75" : "border-line"}`}>
    ...
    {!archived && updateLead ? <details className="mt-3 border-t border-line pt-3"><summary className="cursor-pointer inline-flex items-center gap-1.5 text-[11px] font-bold text-tide"><ClipboardEditIcon />تعديل بيانات الطلب</summary><LeadEditForm lead={lead} updateLead={updateLead} /></details> : null}
...
export function LeadsPage({ leads, createLead, updateLead, archiveLead, createActivity, createFollowUp, completeFollowUp, convertLead }: LeadsPageProps) {
