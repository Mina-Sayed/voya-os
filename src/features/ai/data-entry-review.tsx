      ...
      {payload.properties.map((property, index) => {
        ...
        return <article className={`rounded-2xl border border-[#dbe7e0] bg-white p-4 ${propertyFieldsDisabled ? "opacity-70" : ""}`} key={`property-${index}`}>
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            ...
            <label className="text-[10px] font-bold text-harbor">المنطقة الزمنية<input aria-label={`المنطقة الزمنية للعقار ${index}`} className="mt-1 h-10 w-full rounded-lg border border-line px-2 text-xs outline-none focus:border-tide" disabled={propertyFieldsDisabled} onChange={(event) => updateProperty(index, "timezone", event.target.value)} value={textValue(property.timezone)} /></label>
