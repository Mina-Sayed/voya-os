...
import { isSupportedTimezone, SUPPORTED_TIMEZONES } from "@/domain/time/timezone-contract";
    ...
    <form action={confirmAction} className="mt-5 space-y-4" ref={confirmFormRef}>
      ...
      {payload.properties.map((property, index) => {
        ...
        return <article className={`rounded-2xl border border-[#dbe7e0] bg-white p-4 ${propertyFieldsDisabled ? "opacity-70" : ""}`} key={`property-${index}`}>
          ...
          {failureMessage ? <p className="mt-2 rounded-lg bg-[#fff3ef] px-2 py-1.5 text-[10px] font-semibold leading-5 text-coral">{failureMessage}</p> : null}
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <label className="text-[10px] font-bold text-harbor">كود العقار<input aria-label={`كود العقار ${index}`} className="mt-1 h-10 w-full rounded-lg border border-line px-2 text-xs outline-none focus:border-tide" disabled={propertyFieldsDisabled} onChange={(event) => updateProperty(index, "code", event.target.value)} value={textValue(property.code)} /></label>
            <label className="text-[10px] font-bold text-harbor">اسم العقار<input aria-label={`اسم العقار ${index}`} className="mt-1 h-10 w-full rounded-lg border border-line px-2 text-xs outline-none focus:border-tide" disabled={propertyFieldsDisabled} onChange={(event) => updateProperty(index, "name", event.target.value)} value={textValue(property.name)} /></label>
            <label className="text-[10px] font-bold text-harbor">المنطقة أو المدينة<input aria-label={`مدينة العقار ${index}`} className="mt-1 h-10 w-full rounded-lg border border-line px-2 text-xs outline-none focus:border-tide" disabled={propertyFieldsDisabled} onChange={(event) => updateProperty(index, "city", event.target.value)} value={textValue(property.city)} /></label>
            <label className="text-[10px] font-bold text-harbor">العنوان<input aria-label={`عنوان العقار ${index}`} className="mt-1 h-10 w-full rounded-lg border border-line px-2 text-xs outline-none focus:border-tide" disabled={propertyFieldsDisabled} onChange={(event) => updateProperty(index, "address", event.target.value)} value={textValue(property.address)} /></label>
            <label className="text-[10px] font-bold text-harbor">المنطقة الزمنية<select aria-label={`المنطقة الزمنية للعقار ${index}`} className="mt-1 h-10 w-full rounded-lg border border-line bg-white px-2 text-xs outline-none focus:border-tide" disabled={propertyFieldsDisabled} onChange={(event) => updateProperty(index, "timezone", event.target.value)} value={textValue(property.timezone)}><option value="">اختر المنطقة الزمنية</option>{property.timezone && !isSupportedTimezone(property.timezone) ? <option value={property.timezone}>{property.timezone} — غير مدعومة</option> : null}{SUPPORTED_TIMEZONES.map((timezone) => <option key={timezone} value={timezone}>{timezone}</option>)}</select></label>
            <label className="text-[10px] font-bold text-harbor">رقم أو اسم الوحدة<input aria-label={`رقم أو اسم وحدة العقار ${index}`} className="mt-1 h-10 w-full rounded-lg border border-line px-2 text-xs outline-none focus:border-tide" disabled={propertyFieldsDisabled} onChange={(event) => updateProperty(index, "unitLabel", event.target.value)} value={textValue(property.unitLabel)} /></label>
            <label className="text-[10px] font-bold text-harbor">عدد غرف النوم<input aria-label={`عدد غرف نوم العقار ${index}`} className="mt-1 h-10 w-full rounded-lg border border-line px-2 text-xs outline-none focus:border-tide" disabled={propertyFieldsDisabled} min={0} onChange={(event) => updatePropertyNumber(index, "bedrooms", event.target.value)} type="number" value={property.bedrooms ?? ""} /></label>
            <label className="text-[10px] font-bold text-harbor">الحد الأقصى للضيوف<input aria-label={`الحد الأقصى لضيوف العقار ${index}`} className="mt-1 h-10 w-full rounded-lg border border-line px-2 text-xs outline-none focus:border-tide" disabled={propertyFieldsDisabled} min={1} onChange={(event) => updatePropertyNumber(index, "maxGuests", event.target.value)} type="number" value={property.maxGuests ?? ""} /></label>
            <label className="text-[10px] font-bold text-harbor sm:col-span-2">ملاحظات التشغيل<textarea aria-label={`ملاحظات تشغيل العقار ${index}`} className="mt-1 min-h-20 w-full rounded-lg border border-line p-2 text-xs outline-none focus:border-tide" disabled={propertyFieldsDisabled} onChange={(event) => updateProperty(index, "operationalNotes", event.target.value)} value={textValue(property.operationalNotes)} /></label>
          </div>
          {review.inputs.length > 0 ? <fieldset className="mt-4 rounded-xl border border-dashed border-[#bfd1cb] p-3"><legend className="px-1 text-[10px] font-bold text-tide">الصور المرجعية لهذا العقار</legend><div className="grid gap-2 sm:grid-cols-2">{review.inputs.map((input) => {
            ...
            return <label className="rounded-lg bg-[#f8fbf9] p-2 text-[10px] text-muted" htmlFor={`image-${index}-${input.id}`} key={input.id}>
              {input.status === "active" ? <Image alt={`معاينة صورة الإدخال ${input.id.slice(0, 8)}`} className="mb-2 h-28 w-full rounded-md border border-line object-cover" height={112} src={previewUrl} unoptimized width={180} /> : <div className="mb-2 grid h-28 w-full place-items-center rounded-md border border-line bg-[#f2f3f3] px-3 text-center text-[10px] font-bold text-muted">{input.status === "mapped" ? "تم نقل الصورة إلى صور العقار" : "الصورة مؤرشفة وغير متاحة"}</div>}
