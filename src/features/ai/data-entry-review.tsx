...
function DataEntryTerminalReview({ review }: Readonly<{ review: DataEntryDraftReview }>) {
  ...
  return <section aria-labelledby={`review-${review.id}`} className="mt-6 rounded-[1.75rem] border border-[#d4dfda] bg-[#f5faf7] p-4 sm:p-6">
    <div className="flex items-start gap-3">
      ...
      <div>
        <p className="text-[10px] font-bold tracking-[0.06em] text-tide">{review.status === "applied" ? "اكتمل الحفظ" : "التأكيد قيد التنفيذ"}</p>
        ...
        <p className="mt-2 text-[11px] leading-5 text-muted">
          {review.status === "applied"
            ? `هذه المسودة للقراءة فقط. تم حفظ ${appliedClients} عميل و${appliedProperties} عقار.`
            : "تم امتلاك تنفيذ التأكيد بالفعل. أعد تحميل الصفحة بعد اكتمال التنفيذ قبل أي محاولة جديدة."}
        </p>
      </div>
    </div>
  </section>;
...
function DataEntryReviewForm({ confirmDraft, rejectDraft, review }: DataEntryReviewProps) {
  ...
  const [includedClients, setIncludedClients] = useState<Set<number>>(() => new Set(review.payload.clients.map((_item, index) => index)));
  const [includedProperties, setIncludedProperties] = useState<Set<number>>(() => new Set(review.payload.properties.map((_item, index) => index)));
  ...
  const canConfirm = canConfirmDataEntryPayload(pendingSelected)
    && (pendingSelected.clients.length > 0 || pendingSelected.properties.length > 0 || hasApplied);
  ...
  return <section aria-labelledby={`review-${review.id}`} className="mt-6 rounded-[1.75rem] border border-[#d4dfda] bg-[#f5faf7] p-4 sm:p-6">
    ...
    {review.status === "partially_applied" ? <p className="mt-4 flex items-start gap-2 rounded-xl border border-[#ead9a8] bg-[#fff9e8] p-3 text-[11px] leading-5 text-[#765d22]"><AlertTriangle aria-hidden="true" className="mt-0.5 size-4 shrink-0" />تم حفظ جزء من المسودة. العناصر المكتملة مقفلة؛ راجع العناصر المتبقية ثم أعد التأكيد.</p> : <p className="mt-4 flex items-start gap-2 rounded-xl border border-[#dbe7e0] bg-white/70 p-3 text-[11px] leading-5 text-muted"><CircleAlert aria-hidden="true" className="mt-0.5 size-4 shrink-0 text-tide" />لم يتم الحفظ بعد؛ هذه مسودة قابلة للتعديل.</p>}
    ...
    <form action={confirmAction} className="mt-5 space-y-4" ref={confirmFormRef}>
      ...
      {payload.properties.map((property, index) => {
        ...
        return <article className={`rounded-2xl border border-[#dbe7e0] bg-white p-4 ${disabled ? "opacity-70" : ""}`} key={`property-${index}`}>
          ...
          {review.inputs.length > 0 ? <fieldset className="mt-4 rounded-xl border border-dashed border-[#bfd1cb] p-3"><legend className="px-1 text-[10px] font-bold text-tide">الصور المرجعية لهذا العقار</legend><div className="grid gap-2 sm:grid-cols-2">{review.inputs.map((input) => <label className="flex items-center gap-2 rounded-lg bg-[#f8fbf9] px-2 py-2 text-[10px] text-muted" htmlFor={`image-${index}-${input.id}`} key={input.id}><input checked={property.imageInputIds.includes(input.id)} id={`image-${index}-${input.id}`} aria-label={`ربط الصورة ${input.id} بالعقار ${index}`} disabled={disabled || (input.status === "mapped" && input.mappedPropertyId !== null)} onChange={(event) => toggleImage(index, input.id, event.target.checked)} type="checkbox" /><span>صورة <bdi dir="ltr">{input.id.slice(0, 8)}</bdi> · {Math.ceil(input.byteSize / 1024)}KB</span></label>)}</div></fieldset> : null}
        </article>;
      })}
...
export function DataEntryReview(props: DataEntryReviewProps) {
  if (props.review.status === "applied" || props.review.status === "confirmed") return <DataEntryTerminalReview review={props.review} key={props.review.id} />;
