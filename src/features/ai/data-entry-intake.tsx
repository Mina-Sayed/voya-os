...
export function DataEntryIntake({ confirmDraft, createDraft, drafts, rejectDraft, reviews = [], submitDraft }: Readonly<{ confirmDraft?: DataEntryAction; createDraft: DataEntryAction; drafts: readonly DataEntryDraftSummary[]; rejectDraft?: DataEntryAction; reviews?: readonly DataEntryDraftReview[]; submitDraft: DataEntryAction }>) {
  ...
  const selectedDraftId = createState.status === "success" && createState.draftId ? createState.draftId : activeDraftId ?? drafts[0]?.id ?? null;
  ...
  return <section aria-labelledby="data-entry-heading" className="mt-10 rounded-[2rem] border border-[#d4dfda] bg-surface p-5 shadow-[0_12px_30px_rgba(16,33,38,0.04)] sm:p-7">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div className="flex gap-3"><div className="grid size-11 shrink-0 place-items-center rounded-2xl bg-harbor text-sea-glass"><Sparkles aria-hidden="true" className="size-5" /></div><div><p className="text-[10px] font-bold tracking-[0.08em] text-tide">مسار محكوم</p><h2 className="mt-1 text-2xl font-extrabold tracking-[-0.08em] text-harbor" id="data-entry-heading">إدخال بيانات بمساعدة Gemini</h2><p className="mt-2 max-w-2xl text-xs leading-6 text-muted">اكتب بيانات العملاء أو العقارات وارفع الصور المرجعية. سيعود النظام بمسودة قابلة للتعديل، ولن يُحفظ أي عميل أو عقار قبل مراجعتك وتأكيدك.</p></div></div>
    ...
    {drafts.length > 0 ? <div className="mt-6 border-t border-line pt-5"><p className="text-[10px] font-bold text-tide">المسودات الأخيرة</p><div className="mt-3 grid gap-2">{drafts.slice(0, 5).map((draft) => <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-line bg-white px-3 py-3" key={draft.id}><div><span className="text-[11px] font-bold text-harbor">{statusLabel[draft.status]}</span><span className="ms-2 text-[10px] text-muted">· {inputCountLabel(draft.inputCount)}</span></div>{draft.id !== activeDraftId ? <button className="min-h-9 rounded-lg border border-line px-3 text-[10px] font-bold text-tide hover:border-tide" onClick={() => setActiveDraftId(draft.id)} type="button">متابعة المسودة</button> : <CheckCircle2 aria-hidden="true" className="size-4 text-tide" />}</div>)}</div></div> : null}
  </section>;
}
