...
export function DataEntryIntake({ canWriteProperties = true, confirmDraft, createDraft, drafts, rejectDraft, reviews = [], submitDraft }: Readonly<{ canWriteProperties?: boolean; confirmDraft?: DataEntryAction; createDraft: DataEntryAction; drafts: readonly DataEntryDraftSummary[]; rejectDraft?: DataEntryAction; reviews?: readonly DataEntryDraftReview[]; submitDraft: DataEntryAction }>) {
  ...
  return <section aria-labelledby="data-entry-heading" className="mt-10 rounded-[2rem] border border-[#d4dfda] bg-surface p-5 shadow-[0_12px_30px_rgba(16,33,38,0.04)] sm:p-7">
    ...
    {activeDraft && confirmDraft && rejectDraft ? (reviews.find((review) => review.id === activeDraft.id) ? <DataEntryReview canWriteProperties={canWriteProperties} confirmDraft={confirmDraft} rejectDraft={rejectDraft} review={reviews.find((review) => review.id === activeDraft.id)!} /> : null) : null}
