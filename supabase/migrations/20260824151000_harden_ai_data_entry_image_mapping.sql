-- Atomically register a confirmed AI intake image as a property image and mark
-- the intake input as mapped. This prevents an active property_images row from
-- surviving a failed second transaction between registration and mapping.
...
BEGIN
  ...
  -- Shared confirmation lock order: draft first, then input, then property.
...
REVOKE ALL ON FUNCTION public.apply_ai_data_entry_property_image_v1(uuid,uuid,uuid,uuid,text,text,bigint,integer,integer,text,uuid,uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.apply_ai_data_entry_property_image_v1(uuid,uuid,uuid,uuid,text,text,bigint,integer,integer,text,uuid,uuid) TO service_role;
