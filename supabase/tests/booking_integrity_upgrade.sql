...
DO $$
BEGIN
  IF (SELECT payload_hash IS NOT NULL
    ...
    RAISE EXCEPTION 'legacy completion key was not backfilled with a stable payload hash';
  END IF;
END;
$$;
...
-- Retrying the pre-migration key returns its original success and must not
-- mutate the newer terms now stored on the draft.
SELECT public.complete_booking_commercial_snapshot(
  'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
...
-- A legacy key whose terms cannot be reconstructed is fail-closed.
