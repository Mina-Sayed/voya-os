...
BEGIN
  BEGIN
    PERFORM public.complete_booking_commercial_snapshot(
      'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
      'aaaaaaaa-0000-0000-0000-000000000430', '300000', 'EGP',
      'different reason', 'booking-integrity-completion-430', NULL
    );
    RAISE EXCEPTION 'commercial completion key accepted a different reason';
...
-- A successful completion remains idempotent after the booking advances to
-- pending approval. The stored completion result must be checked before the
-- draft-only state guard.
SELECT set_config('request.jwt.claim.sub', '11111111-1111-1111-1111-111111111111', false);
SELECT set_config('request.jwt.claim.aal', 'aal2', false);
SELECT public.request_booking_approval(
  'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
  'aaaaaaaa-0000-0000-0000-000000000430',
  'booking-integrity-approval-after-completion-430', NULL
);
SELECT public.complete_booking_commercial_snapshot(
  'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
  'aaaaaaaa-0000-0000-0000-000000000430', '300000', 'EGP',
...
SELECT public.request_booking_approval(
  'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
  'aaaaaaaa-0000-0000-0000-000000000440', 'booking-integrity-stale-pending-1', NULL
) AS replay_pending_id \gset
SELECT set_config('voya.test.booking_integrity_replay_pending_id', :'replay_pending_id', false);
RESET ROLE;
...
BEGIN
  IF current_setting('voya.test.booking_integrity_fresh_pending_id')::uuid
    ...
    OR current_setting('voya.test.booking_integrity_replay_pending_id')::uuid
       <> current_setting('voya.test.booking_integrity_stale_pending_id')::uuid
    OR (SELECT result_id FROM public.booking_command_idempotency
        WHERE organization_id = 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa'
          AND command_name = 'booking.approval.request'
          AND idempotency_key = 'booking-integrity-stale-pending-1')
       <> current_setting('voya.test.booking_integrity_stale_pending_id')::uuid
    OR (SELECT result_id FROM public.booking_command_idempotency
        WHERE organization_id = 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa'
          AND command_name = 'booking.approval.request'
          AND idempotency_key = 'booking-integrity-stale-pending-2')
       <> current_setting('voya.test.booking_integrity_fresh_pending_id')::uuid
