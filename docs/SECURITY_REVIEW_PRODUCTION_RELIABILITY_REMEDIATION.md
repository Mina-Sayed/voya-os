...
- Protected routes are rejected by the production test if present in the prerender manifest or returned with shared-cache markers.
...
4. **Moderate — scanner evidence is environment-blocked.** Local Trivy DB download and Snyk authentication must pass in CI before release.
