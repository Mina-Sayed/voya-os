...
- Unexpected provider failures are logged with a request ID and returned as retryable state; validation and authorization errors remain distinct so users are not told that an infrastructure outage is bad input.
