...
- The gate accepts an installed scanner only when shell-independent lookup resolves a real executable beneath `/usr/bin`, `/usr/local/bin`, or `/opt`; the file and every ancestor directory must be root-owned and not group/world writable, and version output must validate. Installed Trivy must be exactly `0.67.2`. The fallback is the same version, pinned to the multi-architecture image digest `docker.io/aquasec/trivy@sha256:e2b22eac59c02003d8749f5b8d9bd073b62e30fefaef5b7c8371204e0a4b0c08`.
...
- `npm test`: 54 files, 253 tests passed.
- `npm run test:coverage`: passed; 94.62% statements, 95.78% lines, and 97.97% functions (79.83% branches). The added callback, password-sign-in, Supabase rate-limit-code handling, magic-link cooldown, role-aware navigation, mobile-navigation, runtime-health, task, transport, AI, WhatsApp, strict timestamp, development-CSP, and safe operational cause-code cases raise changed command-boundary coverage without adding production-only branches.
- `npm run lint`, `npm run build`, `npm run test:production`, and seven production-render unit checks passed; every app route is dynamic and protected routes remain private.
...
- `VOYA_AUTH_E2E_DISPOSABLE=1 npm run test:e2e:auth-local`: nine authenticated browser checks passed against an isolated Supabase/Next.js stack, including booking creation through maker-checker approval, confirmation, check-in, and check-out, a real transport Server Action/RPC submission, every protected workspace route, password sign-in, session refresh, membership selection, forged-cookie rejection, suspension, and sign-out; disposable cleanup also passed after the outbox writes.
- Visual QA: reviewed the authenticated transport workspace at desktop and 390px mobile widths; no horizontal overflow or clipping was observed. Generated screenshots are retained under `output/playwright/release-readiness/`.
...
- Managed Supabase migration check: `npx supabase db push --linked --include-all` applied the ten pending migrations plus the booking-approval lint cleanup on 2026-08-02 after the approved window and backup confirmation. A follow-up `npx supabase migration list --linked` showed every local migration matching remote, and `npx supabase db lint --linked` reported no schema errors. The backup/restore drill and managed Preview smoke remain required release evidence.
