...
- `npm run test:coverage`: passed; 94.58% statements, 95.70% lines, and 97.90% functions (79.51% branches). The added callback, password-sign-in, mobile-navigation, runtime-health, task, transport, AI, WhatsApp, and strict timestamp cases raise changed command-boundary coverage without adding production-only branches.
