...
- `npm run test:coverage`: passed; 94.60% statements, 95.76% lines, and 97.96% functions (79.71% branches). The added callback, password-sign-in, Supabase rate-limit-code handling, magic-link cooldown, role-aware navigation, mobile-navigation, runtime-health, task, transport, AI, WhatsApp, strict timestamp, and development-CSP cases raise changed command-boundary coverage without adding production-only branches.
