...
- Managed Supabase migration check: `npx supabase db push --linked --include-all` applied the ten pending migrations on 2026-08-02 after the approved window and backup confirmation. A follow-up `npx supabase migration list --linked` showed every local migration matching remote. The backup/restore drill and managed Preview smoke remain required release evidence.
