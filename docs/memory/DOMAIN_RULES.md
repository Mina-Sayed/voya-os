...
- `complete_booking_commercial_snapshot` is a draft-only, idempotent completion command. Confirmed or later rows use the approved amendment flow where applicable.
- Legacy `request_booking_approval` and `confirm_booking` require a workspace AAL2 JWT at the database boundary; stale active approval snapshots are cancelled and replaced on retry.
