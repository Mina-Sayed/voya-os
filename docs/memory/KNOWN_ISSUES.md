...
| K-043 | Partially resolved — checkout | Booking amendment/cancellation RPCs are now wired to Server Actions + workspace controls (amendment request/execute already existed; draft cancel, cancellation request/execute added) and the approvals UI decides `booking.confirm`/`amend`/`cancel`. Remaining: `list_executable_booking_changes_v1` projects confirm/amend only, so the execute-cancellation control renders only when the projection yields a cancel row (dormant with the current DB); cancellation finance effects remain open policy | `src/app/workspace/bookings/actions.ts`, `src/features/bookings/bookings-page.tsx`, `src/app/workspace/bookings/page.tsx`, `src/features/bookings/executable-amendments.ts` + colocated tests; projection scope in `supabase/migrations/20260827020000_list_executable_booking_changes_v1.sql` |
