...
|---|---|---|---|
| K-050 | P1 / Verified — checkout; matching managed staging definitions/grants | Exposed property RPCs accept AAL1 despite workspace AAL2 policy | Local read/create proof with explicit AAL1 claims, rolled back; review R-01 |
| K-051 | P1 / Verified — checkout; managed staging EXECUTE grants verified | Legacy booking commands still permit sales confirmation without amount/currency after independent approval | Local complete legacy lifecycle proof, rolled back; review R-02 |
| K-052 | P1 / Verified — checkout | Commercial booking UI mixes major-unit labels with raw minor-unit input/display | Draft form, action, card and approval rendering; review R-04 |
| K-053 | P1 for transport / Verified — checkout | Offset-free transport datetime is interpreted in server timezone | datetime-local form → parseIsoDateTime; identical input differs by three hours for UTC/Cairo on 2026-09-05; review R-07 |
...
| K-024 | Verified — managed Vercel snapshot | Production correlates to a clean four-argument artifact; the relevant preview is dirty/ambiguous | Read-only 2026-08-05 evidence: production `ac7dfdb…` is clean and calls four args; preview reports HEAD with `gitDirty=1`; no provider mutation was performed |
| K-025 | Verified — checkout | Overall security scanner gate is blocked by unavailable Snyk tooling | Trivy passed with zero findings; `npm run scan:security` reports Snyk `BLOCKED` and overall `BLOCKED` |
