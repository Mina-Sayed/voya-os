...
| K-025 | Verified — checkout | Overall security scanner gate is blocked by unavailable Snyk tooling | `npm run scan:security` reports Snyk `BLOCKED` and overall `BLOCKED`/`FAIL`, never PASS (guard enforced by `--self-test`, incl. `missing_snyk_is_blocked`). Phase 0.4 (2026-09-03, checkout): quality.yml runs the self-test as evidence, probes the Snyk credential via `env` into `steps.snyk_auth.outputs.available` (the `secrets` context is rejected inside `if:`), runs the Snyk action only when present, and records `BLOCKED/authentication_missing` then fails the required verify job when absent (findings still fail when the token is present), while Trivy (`exit-code: 1`) + `npm audit` remain enforcing. In the Phase 0.4 worktree Trivy could not run (no trusted binary; container path needs a docker socket this env denies) and `npm audit --omit=dev --audit-level=high` reports 0 vulnerabilities |
