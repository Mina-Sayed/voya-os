...
- **Verified — managed Vercel:** there were no grouped runtime errors in the most recent 6 hours. The 24-hour range contained one `/auth/callback` failure (`pkce_code_verifier_not_found`) on older deployment `dpl_5LbksWfGzpEVg5FexBpCYmLjbT7C` / commit `f6642d2`, at `2026-09-22T03:00:18Z`; cause is unknown and it is not evidence of a current repeated failure. Raw error-log query returned no individual lines. Supabase Auth platform logs were not read in this check.
