...
`npm run dev:local` starts the dedicated local Supabase stack, applies pending
forward migrations, waits for the Auth health endpoint, reads the publishable
and service-role keys from `127.0.0.1:55321`, generates ephemeral server-only
development secrets, and starts Next.js at `127.0.0.1:3102`. It never resets the
local database or reads/forwards managed environment credentials. If the local
migration history belongs to another checkout, inspect it with
`supabase migration list --local` and reset only when the local data is
disposable. Use `npm run dev` only when supplying a complete environment
